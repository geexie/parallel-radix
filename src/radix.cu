...
{
    public:
    __host__ __device__ CudaClass(float newA = 0.f): a(newA)
    {
#if defined __CUDA_ARCH__
        printf("constructor DEVICE\n");
#else
        printf("constructor HOST\n");
#endif
    };

    __host__ __device__ CudaClass(const CudaClass& other): a(other.a)
    {
#if defined __CUDA_ARCH__
        printf("constructor copy DEVICE\n");
#else
        printf("constructor copy HOST\n");
#endif
    };

    __host__ __device__ ~CudaClass()
    {
#if defined __CUDA_ARCH__
        printf("destructor DEVICE\n");
#else
        printf("destructor HOST\n");
#endif
    };
...
// __global__ void useClass(CudaClass cudaClass, float a) {

//     float b = cudaClass(a);
//     printf("Value on Device: %f\n", b);
// }

template<typename T, int num_warps, int log_warps>
__global__ void ballot_scan_radix(T *a_p, T *d_p , int len, CudaClass add42)
{
    int shift = 0;
    typedef typename radix::radix_traits<T>::integer_type I;
    CudaClass cudac;
...
{
    CudaClass add42(42.0f);

    // float _a = 10;
    // float b = add42(_a);

    // printf("Value on Host: %f\n", b);

    // useClass<<<1,1>>>(add42, _a);
    // return 0;
    ...
    switch (len)
    {
        case 32  : ballot_scan_radix<T, 1,  0><<<1,      32>>>(a, d, threads, add42);  break;
        case 64  : ballot_scan_radix<T, 2,  1><<<1,      64>>>(a, d, threads, add42);  break;
        case 128 : ballot_scan_radix<T, 4,  2><<<1,      128>>>(a, d, threads, add42); break;
        case 256 : ballot_scan_radix<T, 8,  3><<<1,      256>>>(a, d, threads, add42); break;
        case 512 : ballot_scan_radix<T, 16, 4><<<groups, 512>>>(a, d, threads, add42); break;
        default  : ballot_scan_radix<T, 16, 4><<<groups, 512>>>(a, d, threads, add42); break;
    }
